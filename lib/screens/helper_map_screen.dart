...
class HelperMapScreen extends StatefulWidget {
  ...
  const HelperMapScreen({this.targetLat, this.targetLng, Key? key}) : super(key: key);
...
class _HelperMapScreenState extends State<HelperMapScreen> {
  ...
  Future<void> _initialize() async {
    ...
    if (widget.targetLat != null && widget.targetLng != null) {
      ...
      if (mounted) {
        ...
        controller.animateCamera(CameraUpdate.newLatLngZoom(targetPos, 17));
      }

      setState(() {}); // Refresh with new marker
    }
  }

  void _addTargetMarker() {
    if (widget.targetLat != null && widget.targetLng != null) {
      final targetPos = LatLng(widget.targetLat!, widget.targetLng!);
  ...
  void _addOrUpdateUserMarker(LatLng pos) {
    ...
    _addTargetMarker();  // Re-add orange target pin every time
  ...
  void _listenToSOSAlerts() {
    FirebaseFirestore.instance
      ...
      setState(() {
        ...
        _addTargetMarker();  // Re-add Target marker to preserve it
      });
    });
  }
  ...
  void _loadNGOMarkers() async {
    ...
    _updateETAandProgress();
    setState(() {
      _addTargetMarker();  // Re-add Target marker
    });
  }
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: const [
                      ...
                      SizedBox(width: 6),
                      Text("Target Location", style: TextStyle(fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ),
